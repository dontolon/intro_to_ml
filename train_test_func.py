...
def train_net(x_train, y_train, x_val, y_val, backup_folder, 
              ...
              print_top_stats_per_epoch=True):
    ...
    for epoch in range(num_epochs):
        ...
        for tr_count, y in enumerate(train_loader):
            net.train()
            x = y[:, :-1].type(t.Tensor).to(device)
            label = y[:, -1].long().to(device)
            opt.zero_grad()
            out = net.forward(x)
        ...
        for idx, data in enumerate(val_loader):
            net.eval()
            x = data[:, :-1].type(t.Tensor).to(device)
            label = data[:, -1].long()
            opt.zero_grad()
            out = net.forward(x)
            val_cum_loss += criterion(out, label.to(device)).item()
...
def test_net(x_test, model):
    ...
    for idx, data in enumerate(test_loader):
        x = data[:, :2].type(t.Tensor).to(device)
        out = net.forward(x)
