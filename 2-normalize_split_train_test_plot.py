...
scen_idxs = [3,4] #np.arange(1,9+1)   # [1,2,3,4,5,6,7,9]
...
train_val_test_split = [80,0,20]     # 0 on val uses test set to validate.    
top_beams = np.arange(30) + 1          # Collect stats for Top X predictions
...
for scen_idx, n_beams, norm_type, noise, rep in combinations:
    ...
    for ai_strategy in ai_strategies:
        ...
        def compute_mean_power_loss(n_test_samples, y_test_pwr, pred_beams, top_beams):
            mean_power_losses_db = np.zeros(len(top_beams))
            for j, k in enumerate(top_beams):
                power_loss_ratio = np.zeros(n_test_samples)
                for i in range(n_test_samples):
                    noise = np.min(y_test_pwr[i, :]) / 2

                    # Use the maximum power from the top-k predicted beams
                    pred_pwr = np.max(y_test_pwr[i, pred_beams[i][:k]])
                    true_pwr = np.max(y_test_pwr[i, :])

                    if pred_pwr == noise:
                        noise = noise / 2

                    power_loss_ratio[i] = (true_pwr - noise) / (pred_pwr - noise)

                mean_power_losses_db[j] = 10 * np.log10(np.mean(power_loss_ratio))
            return mean_power_losses_db
        
        def count_exceeding_loss(n_test_samples, y_test_pwr, pred_beams, top_beams, threshold=3):
            exceed_percentages = np.zeros(len(top_beams))
            for idx_k, k in enumerate(top_beams):
                exceed_count = 0
                for i in range(n_test_samples):
                    noise = np.min(y_test_pwr[i, :]) / 2

                    # Use the maximum power from the top-k predicted beams
                    pred_indices = pred_beams[i][:k]  # Already zero-based
                    pred_pwr = np.max(y_test_pwr[i, pred_indices])
                    true_pwr = np.max(y_test_pwr[i, :])

                    if pred_pwr == noise:
                        # Adjust noise slightly to avoid division by zero
                        noise = noise / 2

                    power_loss = 10 * np.log10((true_pwr - noise) / (pred_pwr - noise))

                    if power_loss > threshold:
                        exceed_count += 1

                exceed_percentage = (exceed_count / n_test_samples) * 100
                exceed_percentages[idx_k] = exceed_percentage

            return exceed_percentages


        # ----------- Phase 6: Save Accuracies and Power Losses ---------------
        if run_idx <= n_avgs:
            # Adjust y_test and pred_beams to zero-based indexing if not done already
            #y_test = y_test - 1
            #clpred_beams = [np.array(pb) - 1 for pb in pred_beams]

            # Get top-1, top-2, top-3 and top-5 accuracies
            total_hits = np.zeros(n_top_stats)
            for i in range(n_test_samples):
                for idx_k, k in enumerate(top_beams):
                    # Check if true beam is in top-k predicted beams
                    hit = np.any(pred_beams[i][:k] == y_test[i])
                    total_hits[idx_k] += 1 if hit else 0

            # Calculate accuracies
            acc = np.round(total_hits / n_test_samples, 4)
            print(f'{ai_strategy} Results:')
            for idx_k in range(n_top_stats):
                print(f'\tAverage Top-{top_beams[idx_k]} accuracy {acc[idx_k]*100:.6f}%')

            # Save Test accuracies to file
            np.savetxt(os.path.join(run_folder, 'test_accs.txt'), acc * 100, fmt='%.6f')

            # Compute mean power losses for top-k beams
            mean_power_losses_db = compute_mean_power_loss(n_test_samples, y_test_pwr, pred_beams, top_beams)
            for idx_k in range(n_top_stats):
                print(f'Mean power loss for top-{top_beams[idx_k]} beams: {mean_power_losses_db[idx_k]:.6f} dB')

            # Save mean power losses to file
            np.savetxt(os.path.join(run_folder, 'power_loss.txt'), mean_power_losses_db, fmt='%.6f')

            # Compute exceed percentages
            threshold = 3  # Adjust threshold as needed
            exceed_percentages = count_exceeding_loss(n_test_samples, y_test_pwr, pred_beams, top_beams, threshold)
            for idx_k in range(n_top_stats):
                print(f'Percentage of samples with power loss > {threshold} dB for top-{top_beams[idx_k]} beams: {exceed_percentages[idx_k]:.6f}%')

            # Save exceed percentages to file
            np.savetxt(os.path.join(run_folder, 'exceed_percentages.txt'), exceed_percentages, fmt='%.6f')
